...
If you use a custom domain for the Okta IdP org, the certificate must be Okta-managed. If you use a self-managed certificate or a non-Okta-hosted custom domain, the downstream Okta SP org ignores the `okta_auth` claim in the SAML assertion. See [Use an Okta-managed certificate](/docs/guides/custom-url-domain/main/#use-an-okta-managed-certificate).
