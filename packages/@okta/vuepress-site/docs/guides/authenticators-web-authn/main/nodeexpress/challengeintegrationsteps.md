...
If you configure your Okta org as detailed in [Configuration updates](#update-configurations) and the Passkeys is already [enrolled](#integrate-sdk-for-authenticator-enrollment) for the user, `authenticate()` returns a response indicating that Passkeys is required for verification. Specifically, `IdxTransaction` is returned with a `status` of `PENDING` and `nextStep.name` set to `challenge-authenticator`.
...
### Get a credential and create a cryptographic signature
...
### Build a parameter for sending the signature
